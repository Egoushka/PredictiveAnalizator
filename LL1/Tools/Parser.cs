...
namespace LL1
{
	...
	internal class Parser
	{
		...
		private readonly IDictionary<string, IDictionary<string, ConfigurationRule>> _parseTable;
		private Tokenizer _tokenizer;
		private readonly IEnumerator<Token> _tokEnum;
		...
		public string Symbol {
			get {
				...
				return s.StartsWith("#END ") ? s.Substring(5) : s;
			}
		}
		/// <summary>
		...
		public Parser(IDictionary<string, IDictionary<string, ConfigurationRule>> parseTable,
			Tokenizer tokenizer,
			string startSymbol
			)
		{
		...
		public bool Read()
		{
			...
			if(_stack.Count > 0)
			{
				string sid = _stack.Peek(); 
				if(sid.StartsWith("#END "))
				{
					_stack.Pop();
					return true;
				}
				if(string.Equals(sid, _tokEnum.Current.Symbol, StringComparison.Ordinal)) // terminal
				{
					// lex the next token
					_tokEnum.MoveNext();
				...
				// non-terminal
				IDictionary<string, ConfigurationRule> d;
				if(_parseTable.TryGetValue(sid, out d))
				{
					...
					if(d.TryGetValue(_tokEnum.Current.Symbol, out rule))
					{
						...
						int ic = rule.Right.Count;
						for (int i = ic - 1; 0 <= i;--i)
						{
							sid = rule.Right[i];
							_stack.Push(sid);
						}
			...
			if ("#EOS" == _tokEnum.Current.Symbol) return false;
			_Panic();
			return true;
		}
		/// <summary>
		...
		public virtual ParseNode ParseSubtree(bool trimEmpties = false)
		{
			if (!Read())
				return null;
			ParserNodeType nn = NodeType;
			if (ParserNodeType.EndNonTerminal==nn)
				return null;

			ParseNode result = new ParseNode();
			...
			if (ParserNodeType.NonTerminal == nn)
			{
				...
				while (true)
				{
					ParseNode k = ParseSubtree(trimEmpties);
					if (null != k)
					{
