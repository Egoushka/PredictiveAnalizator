...
namespace LL1
{
	...
	internal class TokenEnumerator : IEnumerator<Token>
	{
		...
		private readonly ICollection<FA> _initialStates;
		...
		public TokenEnumerator(FA lexer,IEnumerable<char> @string)
		{
			...
			_initialStates = lexer.FillEpsilonClosure();
		...
		private static string _GetAcceptingSymbol(IEnumerable<FA> states)
		{
			return (from fa in states where null != fa.AcceptingSymbol select fa.AcceptingSymbol).FirstOrDefault();
		}
		public void Reset()
		{
			_input.Reset();
