...
namespace LL1
{
	...
	internal class FA : ICloneable
	{
		...
		public IList<FA> FillClosure(IList<FA> result = null)
		{
			...
			result.Add(this);
			foreach (FA fa in Transitions.Values)
				fa.FillClosure(result);
			foreach (FA fa in EpsilonTransitions)
				fa.FillClosure(result);
			return result;
		}
		/// <summary>
		...
		public FA Clone()
		{
			...
			for(int i = 0;i<nclosure.Length;i++)
			{
				nclosure[i] = new FA();
			...
			for (int i = 0; i < nclosure.Length; i++)
			{
				...
				{
					int id = closure.IndexOf(trns.Value);
					t.Add(trns.Key, nclosure[id]);
				}
				foreach (FA trns in closure[i].EpsilonTransitions)
				{
					int id = closure.IndexOf(trns);
					e.Add(nclosure[id]);
				}
			}
			return nclosure[0];
		...
		private static void _Concat(FA lhs,FA rhs)
		{
			FA f = lhs.FirstAcceptingState;
		...
		public static FA Or(IEnumerable<FA> exprs,string accept = "")
		{
			...
			foreach(FA fa in exprs)
			{
				fa.EpsilonTransitions.Add(fa);
				FA nfa = fa.Clone();
				FA nffa = fa.FirstAcceptingState;
		...
		{
			FA result = expr.Clone();
			FA f = result.FirstAcceptingState;
		...
		public FA FirstAcceptingState {
			get
			{
				return FillClosure().FirstOrDefault(fa => null != fa.AcceptingSymbol);
			}
		}
		/// <summary>
		...
		public static ICollection<FA> FillMove(IEnumerable<FA> states, char input,ICollection<FA> result = null) {
			result ??= new List<FA>();
			foreach(FA fa in states)
			{
				// examine each of the states reachable from this state on no input
				foreach(FA efa in fa.FillEpsilonClosure())
				{
					// see if this state has this input in its transitions
					if (!efa.Transitions.TryGetValue(input, out FA ofa)) continue;
